...
import {
  extractEntitiesFromForm10Q,
  mapEntitiesToSignals,
} from "../../../tools/AiAgents/scraperAgents/liquidity/Form10QParserAgent.js";
import { Form10QData, FeedEntry } from "../../../types/signal.types.js";
import { secRequest } from "./commonScraping.service.js";
import * as cheerio from "cheerio";

/**
 * Scrape Form 10-Q from URL and directly convert to Signal(s)
 * without saving to Form10Q database
 * @param url - Direct URL to Form 10-Q filing or index page
 * @returns Object with success status and signal IDs created
 */

export const scrapeForm10QToSignal = async (
  url: string,
): Promise<{ success: boolean; signalIds: string[]; error?: string }> => {
  try {
    console.log(`\n🔄 Scraping Form 10-Q from URL and converting to Signal(s)`);
    console.log(`   URL: ${url}`);
    ...
    const existingSignals = await Signal.find({ accession, signalType: "10-q" }).lean();
    ...
    if (url.includes("-index.htm")) {
      // This is an index page, need to find the actual 10-Q document
      console.log(`📄 Detected index page, fetching to find 10-Q document...`);
      ...
      for (const docUrl of documentLinks) {
        try {
          ...
          const docData = await secRequest<string>(
            ...
            `10-Q document: ${accession}`,
          );

          // Validate this is a 10-Q document (check for quarterly report indicators)
          if (
            docData &&
            docData.length > 1000 &&
            (docData.toLowerCase().includes("quarterly report") ||
              docData.toLowerCase().includes("form 10-q") ||
              docData.toLowerCase().includes("item 1.") ||
              docData.toLowerCase().includes("item 2.") ||
              docData.toLowerCase().includes("part i") ||
              docData.toLowerCase().includes("part ii") ||
              docData.toLowerCase().includes("condensed") ||
              docData.toLowerCase().includes("interim financial"))
          ) {
            ...
            console.log(
              `   ⚠️  Document fetched but doesn't appear to be a 10-Q (${docData.length} chars), trying next...`,
            );
          }
        } catch (err) {
      ...
      if (!documentContent) {
        ...
        try {
          const txtData = await secRequest<string>(txtUrl, {}, `10-Q .txt: ${accession}`);
      ...
      if (!documentContent) {
        return {
          success: false,
          signalIds: [],
          error: "Could not find valid 10-Q document in index",
        };
      }
    } else {
      // Direct document URL
      documentUrl = url;
      console.log(`📥 Fetching 10-Q document...`);
      documentContent = await secRequest<string>(url, {}, `10-Q document: ${accession}`);
    }
    ...
    if (!documentContent) {
      return {
        success: false,
        signalIds: [],
        error: "Failed to fetch 10-Q content",
      };
    }
    ...
    // Try multiple patterns for period end date, prioritizing clean date formats
    let fiscalPeriodEnd = "";
    const periodPatterns = [
      /CONFORMED PERIOD OF REPORT:\s*(\d{8})/i,
      /PERIOD END:\s*(\d{8})/i,
      /period end[:\s]*(\d{4}-\d{2}-\d{2})/i,
      /ended\s+(\w+\s+\d{1,2},\s+\d{4})/i,
      /for the.*?quarter.*?ended[:\s]*(\w+\s+\d{1,2},\s+\d{4})/i,
    ];

    for (const pattern of periodPatterns) {
      const match = documentContent.match(pattern);
      if (match && match[1]) {
        fiscalPeriodEnd = match[1].trim();
        // Remove any HTML tags if present
        fiscalPeriodEnd = fiscalPeriodEnd.replace(/<[^>]+>/g, "").trim();
        if (fiscalPeriodEnd.length > 0 && fiscalPeriodEnd.length < 50) {
          break;
        }
      }
    }
    ...
    console.log(`   Fiscal Period End: ${fiscalPeriodEnd}`);

    // Extract entities from 10-Q using AI agent
    console.log(`🤖 Extracting entities from Form 10-Q...`);
    const entities = await extractEntitiesFromForm10Q(documentContent);

    if (!entities || entities.length === 0) {
      console.warn(`⚠️  No entities extracted from 10-Q: ${accession}`);
      return {
        success: true,
        signalIds: [],
      };
    ...
    // Create a Form10Q object for mapping
    const form10qData: Form10QData = {
      accession,
    ...
    const signals = mapEntitiesToSignals(entities, form10qData);
    ...
    console.error(`❌ Failed to scrape 10-Q to Signal:`, error.message);
    return {
      success: false,
      signalIds: [],
      error: error.message,
...
export const scrapeLatest10QsToSignals = async (
  ...
  try {
    console.log(`\n📡 Fetching latest 10-Q filings from SEC RSS feed (limit: ${limit})`);
    ...
    const rssData = await secRequest<string>(rssUrl, {}, "10-Q RSS feed");

    // Parse RSS feed
    const $ = cheerio.load(rssData, { xmlMode: true });
    const entries: FeedEntry[] = [];

    $("entry").each((_idx, elem) => {
      ...
      // Only process 10-Q filings
      if (category === "10-Q" && link) {
    ...
    console.log(`📋 Found ${entries.length} 10-Q filings to process`);
    ...
    for (const entry of entries) {
      ...
      const result = await scrapeForm10QToSignal(entry.link);
    ...
    console.error(`❌ Failed to scrape latest 10-Qs:`, error.message);
    throw error;
  }
};

/**
 * Scrape historical Form 10-Q filings from SEC for a date range
 * @param fromDate - Start date (YYYY-MM-DD)
 * @param toDate - End date (YYYY-MM-DD)
 * @param maxResults - Maximum number of results to fetch (default: 100, max: 100)
 * @returns Statistics about scraping results
 */
export const scrapeHistorical10QsToSignals = async (
  fromDate: string,
  toDate: string,
  ...
  try {
    console.log(`\n📅 Fetching historical 10-Q filings from ${fromDate} to ${toDate}`);
    console.log(`   Max results: ${maxResults}`);

    // Use SEC EFTS search API for historical data
    const searchUrl = `https://efts.sec.gov/LATEST/search-index?dateRange=custom&category=form-cat1&forms=10-Q&startdt=${fromDate}&enddt=${toDate}&page=1&from=0&size=${maxResults}`;
    ...
    console.log(`📋 Found ${hits.length} 10-Q filings in date range`);
    ...
    const entries: FeedEntry[] = hits.map((hit: any) => {
      ...
      return {
        title: `${source.display_names?.[0] || "Unknown"} - 10-Q`,
        link: `https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK=${cik}&type=10-Q&dateb=${source.file_date}&owner=exclude&count=1`,
    ...
    for (const entry of entries) {
      ...
      const result = await scrapeForm10QToSignal(indexUrl);
    ...
    console.error(`❌ Failed to scrape historical 10-Qs:`, error.message);
    return {
      success: false,
      total: 0,
      successful: 0,
...
/**
 * Get enrichment statistics for 10-Q signals
 * @returns Statistics about 10-Q signal enrichment status
 */
export const get10QSignalEnrichmentStats = async () => {
  const total = await Signal.countDocuments({ signalType: "10-q" });
