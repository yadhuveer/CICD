 ...
 * -----------------------------------
 */
const systemPrompt = `You are a precise SEC Form 10-Q (Quarterly Report) parser.

Extract this JSON as an array (one object per key person/entity mentioned in the 10-Q):
[
  {{
    ...
    "companyName": "<company filing the 10-Q>",
...
Name:
- Extract the full name exactly as it appears in the 10-Q filing.
...
companyName:
- For Individuals: Use the company filing the 10-Q (the issuer)
- For Companies: Use the company filing the 10-Q (the issuer)
- This should be consistent across all entries from the same 10-Q

companyNameVariants:
- IMPORTANT: This field contains variations of the COMPANY NAME (from companyName field), NOT the entity's individual name
- For both Individuals AND Companies: Generate 6-8 realistic variations of the COMPANY FILING THE 10-Q (the issuer)
...
- Look in sections: "Directors and Executive Officers", "Executive Officers", "Item 1", "Item 2", "Part II", "Management's Discussion", "Corporate Governance", "Management"
...
- Typically expect 5-20 entities per 10-Q filing
- Note: 10-Q reports may have less detailed management information than 10-K filings, focus on what's available
 ...
 * -----------------------------------
 */
function preprocessForm10QContent(content: string): string {
  ...
  // Extract key sections using common headings for 10-Q filings
  const sections = [
    "item 1",
    "item 2",
    "item 5",
  ...
  for (const section of sections) {
    ...
    if (index !== -1) {
      ...
      // Extract content around this section (15000 characters window for 10-Q)
 ...
 * -----------------------------------
 */
export async function extractEntitiesFromForm10Q(content: string) {
  try {
    console.log("AI🤖 Model Extracting entities from Form 10-Q...");
    ...
    const processedContent = preprocessForm10QContent(content);
    ...
    console.error("Error extracting entities from Form 10-Q:", err);
    throw err;
  }
}
 ...
 * @param entities - Array of extracted entities from Form 10-Q
 * @param form10qData - The original Form 10-Q filing document
 * @returns Array of signals ready to be saved to DB
 */
/**
 * Helper function to parse period end date from various formats
 */
function parsePeriodEndDate(dateStr: string): Date | undefined {
  if (!dateStr || dateStr.trim().length === 0) {
    return undefined;
  }

  try {
    // Format: YYYYMMDD (e.g., 20250930)
    if (/^\d{8}$/.test(dateStr)) {
    ...
    // Format: Month DD, YYYY (e.g., September 30, 2025)
    const monthDayYear = /^(\w+)\s+(\d{1,2}),\s+(\d{4})$/i.exec(dateStr.trim());
    if (monthDayYear) {
    ...
    console.warn(`Could not parse period end date: ${dateStr}`);
  }

  return undefined;
}

export function mapEntitiesToSignals(entities: any[], form10qData: any) {
  return entities.map((entity) => {
    // Determine if this is a Person or Company signal
    const signalSource = entity.entityType === "Individual" ? "Person" : "Company";

    // Parse period of report date safely
    const periodOfReport = parsePeriodEndDate(form10qData.fiscalPeriodEnd);
    ...
    const signal: any = {
      ...
      // Filing Information from Form 10-Q
      filingType: "Form 10-Q quarterly report",
      companyName: form10qData.companyName || entity.companyName,
      companyTicker: form10qData.companyTicker,
      cik: form10qData.companyCik,
      accession: form10qData.accession,
      filingDate: form10qData.filingDate,
      periodOfReport: periodOfReport,
      filingLink: form10qData.filingLink,
      sourceUrl: form10qData.filingLink,
      ...
      // Source metadata
      sourceOfInformation: "SEC EDGAR - Form 10-Q",
    ...
    if (signalSource === "Company" && entity.keyPeople && entity.keyPeople.length > 0) {
      signal.keyPeople = entity.keyPeople.map((person: any) => ({
        ...
        designation: person.designation,
        location: person.location,
        sourceOfInformation: "SEC Form 10-Q Filing",
        dateAdded: new Date(),
